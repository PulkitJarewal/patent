...
searchButton.addEventListener("click", () => {
  // Gather search inputs from all search bars
  ...
  const finalQuery = queries.join(` ${globalOperator} `);
 ...
 let apiUrl = "https://api.lens.org/patent/search?token=P2bqXKNCDGWxsPcMeZwNCeOQR9gsB9CPYrZFUi33bMilgKQ0b20X";
...
apiUrl += `&size=${resultsPerPage}&from=${currentPageNumber - 1}`;
  ...
  for (const patent of data.data) {
    ...
    const fullTextPromise = getFullText(patent.jurisdiction,patent.doc_number,patent.kind); // Call Get FullText Function
    ...
    <ul>
      ${
        patent.biblio.cited_by.patents && patent.biblio.cited_by.patents.length > 0 
          ? patent.biblio.cited_by.patents.map(patentItem => `
              <li style="display: inline-block">${patentItem.document_id.jurisdiction + patentItem.document_id.doc_number} | </li>
            `).join('')
